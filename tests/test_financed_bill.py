...
def test_apply_daily_interest(financed_bill):
    start_balance = financed_bill.loan_balance
...
def test_make_payment(financed_bill, bank_account):
    ...
    initial_loan_balance = financed_bill.loan_balance
    ...
    assert abs(financed_bill.loan_balance) < abs(initial_loan_balance)
    # Bank account balance reduced
    assert bank_account._ledger.raw_copy_ledger[-1][4] > 0  # debit recorded
...
def test_min_payment_capped(financed_bill, bank_account):
    ...
    financed_bill._accountInfo._balance = -30.0  # small negative balance
    test_date = date(2025, 11, 10)

    financed_bill.make_payment(test_date)

    # Ledger should reduce balance to 0
    assert financed_bill.loan_balance == 0
    # Bank account debited by 30, not 50
    assert bank_account._ledger.raw_copy_ledger[-1][4] == 30
